    ...
    <Card delay={delay} animate={animate} style={{ display:'flex', flexDirection:'column', gap:14 }}>
      ...
      {sub !== undefined && (
        <div style={{ display:'flex', alignItems:'center', gap:6 }}>
          {positive !== undefined && (
            <span style={{ fontSize:11, fontWeight:600, borderRadius:20, padding:'3px 8px', color: positive ? 'oklch(0.68 0.18 145)' : 'oklch(0.64 0.2 25)', background: positive ? 'oklch(0.22 0.08 145)' : 'oklch(0.22 0.08 25)' }}>
              {positive ? '↑' : '↓'} {Math.abs(Number(sub))}%
            </span>
          )}
