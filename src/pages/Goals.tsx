...
function SemiGauge({ pct, color, current }: { pct: number; color: string; current: number }) {
  // Arc path: M10,60 A50,50 0 0,1 110,60  — semicircle, circumference ≈ 157
  const circ = 157
              ...
              <Card key={g.id} delay={i*0.05}>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:16 }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                    ...
                    <button onClick={()=>openEdit(g)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--text-dim)', fontSize:13, padding:'2px 4px' }}>✎</button>
                    <button onClick={()=>deleteGoal(g.id)} style={{ background:'none', border:'none', cursor:'pointer', color:'var(--negative)', fontSize:13, padding:'2px 4px' }}>✕</button>
                  </div>
                </div>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', marginBottom:10 }}>
                  ...
                  <div style={{ textAlign:'center' }}>
                    ...
                    <div style={{ fontSize:14, fontWeight:700, fontFamily:'DM Mono', color: remaining<=0?'var(--positive)':'var(--text)' }}>{remaining<=0?'✓ Done':fmt(remaining, 0)}</div>
                  </div>
                </div>
